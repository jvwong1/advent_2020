...
def read_file(filename):
    # returns a list of joltage ratings
...
def find_invalid_no(input):
    """Find the jolage rating difference and multiply 1 diff and 3 diff"""
    input.sort()  # sort joltage ratings
    joltage = 0  # start at 0
    max_joltage = max(input) + 3
    counter = 0
    while counter < max_joltage:
        print(next(input))
        counter += 1
...
#%%
test = "test.txt"
test2 = "larger_test.txt"
filename = "input.txt"
input = read_file(test)
# input2 = read_file(test2)
# input = read_file(filename)
# Part 1
#%%
try:
    print(find_invalid_no(input))
except:
    print("Error")
#%%
# Part 2
try:
    print(find_invalid_no_2(input))
except:
    print("Error")
